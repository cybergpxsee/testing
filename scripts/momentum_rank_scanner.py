...
# 掃描參數
MIN_LOOKBACK_DAYS = 200  # 至少需要 200 個交易日數據
...
def calculate_momentum_ranks(price_data: dict, spy_data: pd.DataFrame) -> pd.DataFrame:
    ...
    for sym, df in price_data.items():
        ...
        # 距離 200日均線的百分比 (正值 = 價格在均線上方)
        dist_to_sma200 = (price_today / sma_200 - 1.0) * 100.0
...
def main():
    ...
    # SPY 專用最小行數門檻（可比全域更寬鬆，例如 200），不影響其他標的
    SPY_MIN_DAYS = 200

    if spy_yahoo in spy_data_dict and len(spy_data_dict[spy_yahoo]) >= SPY_MIN_DAYS:
        all_price_data = {spy_yahoo: spy_data_dict[spy_yahoo]}
        append_log(stderr_path, f"STAGE2_DOWNLOAD_SPY final success rows={len(spy_data_dict[spy_yahoo])} (>= SPY_MIN_DAYS={SPY_MIN_DAYS})")
    else:
    ...
    append_log(stderr_path, f"MOMENTUM_SCAN_DONE cat1={scan_info['cat1_count']} cat2={scan_info['cat2_count']} cat3={scan_info['cat3_count']} cat4={scan_info['cat4_count']}")
